...
- Effects preserve intent and observation independently. A production model intent pins the run,
  ...
  sufficient. Memory refusal identities are reconstructed from the run-owned expected Belief Event
  and the closed refusal-reason vocabulary. Belief transaction time remains bounded by the lifecycle
  checkpoint; the memory ledger's independent append timestamp may follow it. SQLite initializes or
  validates one exact current physical schema; other non-empty shapes fail before writes.
