...
Decision publication begins only after the exact Balanced result and Conservative, Growth, and
capped equal-weight records are durably complete and semantically reconstructed from their shared
HouseView and portfolio inputs. It freezes one immutable Champion Decision Record for the Market
Session. The record binds the Evidence Cutoff, forecast and Target Band identities, SPY benchmark
state, position snapshot and cash, every shadow identity, Constitution, portfolio and cost policies,
Data Regime, source and model identities, and the complete material fingerprint set. An exact retry
returns that record; later information cannot rewrite it.
