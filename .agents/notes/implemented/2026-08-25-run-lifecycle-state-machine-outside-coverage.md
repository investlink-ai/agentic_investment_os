...
The persistence-level lifecycle state machine dominates `make check` because coverage instrumentation
multiplies the cost of its bounded generated sequences. The remaining deterministic suite already
satisfies every consequence-tier coverage threshold, while the state machine supplies behavioral
evidence for replay, interruption, conflict, reopen, and reference-model agreement.
...
coverage changes must preserve the explicit deselection boundary and tier check.
...
tier while the unchanged state machine passes at its full Hypothesis budget.
